...
ImGuiIO *mk::ImGUIHelper::imgui_io; // define static field

void mk::ImGUIHelper::initialize_context(GLFWwindow *window, const char *glsl_version)
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void mk::ImGUIHelper::render()
{
    ImGui::Render();                                        // prepares data, cant touch imGUI after this
    ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData()); // actually sends data to GPU
}

bool mk::ImGUIHelper::add_mouse_event(int button, bool pressed)
{
    imgui_io->AddMouseButtonEvent(button, pressed);
...
void mk::ImGUIHelper::add_scroll_event(double x, double y)
{
